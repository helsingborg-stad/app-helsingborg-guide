/**
 * Created at Fri May 15 2020 13:40:18 GMT+0200 (Central European Summer Time)
 */

/* eslint-disable max-len */
export default {
  sv: {
    ABOUT: "Om",
    ABOUT_GUIDE_APP: "Om Guide Helsingborg",
    ACCESS_TO_LOCATION: "Tillåt åtkomst till plats",
    ACCESSIBLE: "Handikappanpassat",
    ACTIVITY: "Aktivitet",
    ACTIVITIES: "Aktiviteter",
    ADULT_ONLY: "Enbart vuxna",
    ANIMALS_ALLOWED: "Djur tillåtna",
    APP_NAME: "Guide Helsingborg",
    AFTERNOON: "Efermiddag",
    AR: "Vägvisare",
    AR_CAMERA_DISABLED: "Ge appen tillåtelse att använda kameran",
    AR_CHECKING: "Kontrollerar om funktionen stöds av din telefon",
    AR_HINT_ARRIVED: "Utforska platsen genom att klicka på kortet eller lyssna på ljudguiden",
    AR_HINT_LOOK_DOWN: "Titta ner",
    AR_HINT_LOOK_LEFT: "Titta åt vänster",
    AR_HINT_LOOK_RIGHT: "Titta åt höger",
    AR_HINT_LOOK_UP: "Titta upp",
    AR_HINT_TURN_AROUND: "Vänd dig om",
    AR_INTRO_BETA_BADGE: "Vi testar funktionen och hoppas du ska gilla den!",
    AR_INTRO_BUTTON: "Prova Nu",
    AR_INTRO_INSTRUCTIONS: "Håll upp telefonen och leta efter din valda plats runt omkring dig.",
    AR_INTRO_TITLE: "Ett nytt sätt att utforska Sofiero",
    AR_NOT_SUPPORTED_CALLOUT: "Hoppsan!",
    AR_SUPPORTED: "Funktionen stöds av din telefon",
    AR_TRANSIENT: "Funktionen väntar på att aktiveras",
    AR_UNKNOWN: "Funktionen stöds inte av din telefon. Kontakta oss om du har frågor eller feedback!",
    AR_UNSUPPORTED: "Funktionen stöds inte av din telefon. Kontakta oss om du har frågor eller feedback!",
    BATHING_AREAS: "Badplatser",
    BETA_VERSION: "Beta version",
    BOOKING: "Bokning",
    CALENDAR: "Kalender",
    CALENDAR_FULL_DAY: "Hela dagen",
    CALENDAR_NO_CONTENT: "Det verkar inte finnas några event idag...",
    CANCEL: "Avbryt",
    CHANGES_REQUIRES_APP_RELOAD: "Förändringar kräver app reload",
    CHECK_CAMERA_SERVICE: "Aktivera kamera",
    CHECK_LOCATION_SERVICE: "Aktivera platssjänst",
    CHILDREN_FRIENDLY: "Barnvänligt",
    CHOOSE_LANGUAGE: "Välj språk",
    CINEMA: "Bio",
    CITY_NAME: "Helsingborg",
    CLOSE: "Stäng",
    CLOSE_MORE_INFO: "Stäng mer info",
    CLOSE_TODAY: "Stängt idag",
    COMING_SOON: "Kommer snart",
    CONTACT_MAIL_ADRESS: "kontaktcenter@helsingborg.se",
    CONTACT_MAIL_SUBJECT: "Guide Helsingborg App",
    CONTACT_PHONE: "+4642105000",
    CONTACT_PHONE_DISPLAY: "042-10 50 00",
    CONTACT_US: "Kontakta oss",
    CONTENT_MISSING: "Inget innehåll tillgängligt på ditt valda språk",
    DATE: 'Tidpunkt',
    DELETE_STORED_DATA: "Radera sparad media",
    DETAILED_INFORMATION: "Detaljerad information",
    DISTANCE: "Avstånd",
    DIRECTIONS: "Vägbeskrivning",
    DOWNLOAD: "Ladda ner",
    DOWNLOADS: "Nedladdningar",
    DOWNLOADED: "Tillgängligt offline",
    DOWNLOADED_INFO: "Innehållet är nedladdat och tillgängligt offline. Du kan alltid radera detta från Inställningar.",
    DOWNLOADING: "Laddar ner",
    DOWNLOADING_CANCEL: "Avbryt",
    DOWNLOADING_PAUSE: "(tryck för att pausa)",
    DOWNLOADING_RESUME: "Återuppta",
    EVENING: "Kväll",
    EXERCISE: "Träning",
    EXHIBITIONS: "Utställningar",
    EXPERIENCES:  "Upplevelser",
    INDOOR: "Inomhus",
    INVALID_URL: "Ogiltig URL",
    INVALID_URL_MESSAGE: "Detta innehåll kan inte öppnas i appen",
    FILTER: "Filtrera",
    FILTER_TITLE: "Vad vill du göra idag?",
    FILTER_SETTINGS: "Filterinställningar",
    FOR_CHILDREN: "För barn",
    FROM_HERE: "härifrån",
    FROM_YOU: "från dig",
    GO_BACK: "Gå tillbaka",
    GROCERIES: "Mataffärer",
    GUIDE: "Guide",
    HAS_QUIZ: "Interaktiv guide tillgänglig",
    HI: "hey",
    HOME: "Hem",
    HOUR: "timme",
    INFO: "Info",
    INFORMATION: "Information",
    JUMP_OVER: "HOPPA ÖVER",
    LETS_GO: "SÄTT IGÅNG",
    LIST: "Lista",
    LISTEN: "Spela ljud",
    LOCATION: "Plats",
    LOCATIONS: "platser",
    MAP: "Karta",
    MEDIAGUIDE: "Mediaguide",
    MEDIAGUIDES: "Mediaguider",
    MEDIAGUIDE_INTERACTIVE: "Interaktiv Mediaguide",
    MESSAGE_CAMERA_PERMISSION: "För att fortsätta, ge appen tillgång till din kamera",
    MESSAGE_LOCATION_PERMISSION: "För att fortsätta låt appen använda telefonens platstjänst",
    MINUTE: "min",
    MORNING: "Morgon",
    MOST_POPULAR_ACTIVITIES: "Mest populära aktiviteter",
    MORE_INFO: "Mer info",
    MORE_THAN: "Mer än",
    NEW_CONTENT: "Nytt innehåll",
    NO_BLE_CONNECTION: "Bluetooth avstängd",
    NO_BLE_CONNECTION_MESSAGE: "För full tillgång till Guide Helsingborg behöver du aktivera Bluetooth",
    NO_INTERNET_CONNECTION: "Inget nätverk",
    NO_INTERNET_CONNECTION_MESSAGE: "För full tillgång till Guide Helsingborg behöver du aktivera wifi",
    NOT_FOUND_TITLE: "Hoppsan!",
    NOT_FOUND_DESCRIPTION: "Sidan du letar efter finns inte",
    NOT_FOUND_CONTACT: "Vänligen kontakta support:",
    OBJECT: "objekt",
    OBS: "Något gick fel",
    OFFLINE_CONTENT: "Nedladdat innehåll",
    OFFLINE_CONTENT_TITLE: "Nedladdat",
    OK: "Ok",
    OPEN: "Öppna",
    OPEN_LINK: "Öppna länk?",
    OPEN_IN_MAPS: "Öppna kartan?",
    OPEN_TODAY: "Öppet idag",
    ORGANIZERS: "Arrangörer",
    OUTDOOR: "Utomhus",
    PARKING: "Parkering",
    PADEL_TRACKS: "Padelbanor",
    PLAYING: "Spelar..",
    PLAYGROUNDS: "Lekplatser",
    READ_MORE: "Läs mer!",
    RANDOMIZE: "Slumpa valen",
    RESET: "Återställ",
    SCAN: "Skanna",
    SCAN_QR_CODES: "Skanna QR Koder",
    SCAN_QR_CODES_DESCRIPTION: "Här kan du skanna QR koder som är placerade runtomkring Helsingborg. Håll kameran stilla över QR koden, så öppnas rätt upplevelse i appen!",
    SCAN_QR_CODES_REMINDER: "När du skannar, tänk på att du måste ha samma språk inställt i appen som motsvarar guidens språk. Du kan ändra språk under inställningar.",
    SEARCH: "Sök",
    SEARCH_AROUND: "Nytt innehåll laddas",
    SEARCH_ALL_ACTIVITES: "Sök alla aktiviteter",
    SEARCH_ADDITIONAL_ACTIVITES: "Sök övriga aktiviteter",
    SEARCH_MAIN: "Sök sevärdheter, platser...",
    SEARCH_BY_NUMBER: "Sök efter ID",
    SEE: "Se",
    SEND: "Skicka",
    SETTINGS: "Inställningar",
    SHARE: "Dela",
    SHARING_OVERLAY_TITLE: "Guide Helsingborg App",
    SHOP: "Handla",
    SHOPPING: "Shopping",
    SHOW_MAP: "Visa karta",
    SIGHTSEEINGS: "Sevärdheter",
    SOMETHING_NEAR_BY: "Du är nära",
    TAKE_ME_THERE: "Hitta hit",
    TIME_AND_DATE: "Tid och datum",
    THEATER: "Teater",
    TOPLIST: "Topplistan",
    TOUR: "Rundtur",
    TUTORIAL: "Tutorial",
    UNKNOWN_TITLE: "Okänd titel",
    VIDEO: "Spela film",
    VIEW_ALL: "Se alla",
    WITH: "med",
  },
  en: {
    ABOUT: "About",
    ABOUT_GUIDE_APP: "About Helsingborg Guide",
    ACCESS_TO_LOCATION: "Allow access to location",
    ACCESSIBLE: "Accessible",
    ACTIVITY: "Activity",
    ACTIVITIES: "Activities",
    ADULT_ONLY: "Adult only",
    ANIMALS_ALLOWED: "Animals allowed",
    APP_NAME: "Guide Helsingborg",
    AFTERNOON: "Afternoon",
    AR: "Guide",
    AR_CAMERA_DISABLED: "Camera permission needs to be granted",
    AR_CHECKING: "Checking if this feature is supported",
    AR_HINT_ARRIVED: "",
    AR_HINT_LOOK_DOWN: "Look down",
    AR_HINT_LOOK_LEFT: "Look to the left",
    AR_HINT_LOOK_RIGHT: "Look to the right",
    AR_HINT_LOOK_UP: "Look up",
    AR_HINT_TURN_AROUND: "Turn around",
    AR_INTRO_BETA_BADGE: "",
    AR_INTRO_BUTTON: "Try it out",
    AR_INTRO_INSTRUCTIONS: "Hold your phone up in front of you and look around to find your chosen place.",
    AR_INTRO_TITLE: "A new way to explore Sofiero",
    AR_NOT_SUPPORTED_CALLOUT: "Oops!",
    AR_SUPPORTED: "This feature is supported",
    AR_TRANSIENT: "This feature is waiting to be activated",
    AR_UNKNOWN: "This feature is unsupported. Contact us if you have any questions or feedback!",
    AR_UNSUPPORTED: "This feature is unsupported. Contact us if you have any questions or feedback!",
    BETA_VERSION: "Beta version",
    BATHING_AREAS: "Bathing areas",
    BOOKING: "Booking",
    CALENDAR: "Calendar",
    CALENDAR_FULL_DAY: "All-day",
    CALENDAR_NO_CONTENT: "There don't seem to be any events today...",
    CANCEL: "Cancel",
    CHANGES_REQUIRES_APP_RELOAD: "Changes requires app reload",
    CHECK_CAMERA_SERVICE: "Activate camera",
    CHECK_LOCATION_SERVICE: "Activate location service",
    CHOOSE_LANGUAGE: "Choose a language",
    CHILDREN_FRIENDLY: "For Children",
    CINEMA: "Cinema",
    CITY_NAME: "Helsingborg",
    CLOSE: "Close",
    CLOSE_MORE_INFO: "Close more info",
    CLOSE_TODAY: "Closed today",
    COMING_SOON: "Coming soon",
    CONTACT_MAIL_ADRESS: "kontaktcenter@helsingborg.se",
    CONTACT_MAIL_SUBJECT: "Guide Helsingborg App",
    CONTACT_PHONE: "+4642105000",
    CONTACT_PHONE_DISPLAY: "042-10 50 00",
    CONTACT_US: "Contact us",
    CONTENT_MISSING: "No content available in your selected language",
    DATE: 'Date',
    DELETE_STORED_DATA: "Delete stored data",
    DETAILED_INFORMATION: "Detailed information",
    DISTANCE: "Distance",
    DIRECTIONS: "Directions",
    DOWNLOAD: "Download",
    DOWNLOADS: "Downloads",
    DOWNLOADED: "Available offline",
    DOWNLOADED_INFO: "The content is downloaded and available offline. Go to Settings if you want to remove downloaded content.",
    DOWNLOADING: "Downloading",
    DOWNLOADING_CANCEL: "Cancel",
    DOWNLOADING_PAUSE: "(touch to pause)",
    DOWNLOADING_RESUME: "Resume",
    EVENING: "Evening",
    EXERCISE: "Exercise",
    EXHIBITIONS: "Exhibitions",
    EXPERIENCES: "Experiences",
    INDOOR: "Indoor",
    INVALID_URL: "Invalid URL",
    INVALID_URL_MESSAGE: "This content cant be opened in the app",
    FILTER: "Filter",
    FILTER_TITLE: "Whats on your agenda today?",
    FILTER_SETTINGS: "Filters",
    FOR_CHILDREN: "For children",
    FROM_HERE: "from here",
    FROM_YOU: "from you",
    GO_BACK: "Go back",
    GROCERIES: "Groceries",
    GUIDE: "Guide",
    HAS_QUIZ: "Interactive guide available",
    HI: "hi",
    HOME: "Home",
    HOUR: "hour",
    INFO: "Info",
    INFORMATION: "Information",
    JUMP_OVER: "SKIP",
    LETS_GO: "LET'S GO",
    LIST: "List",
    LISTEN: "Play sound",
    LOCATION: "Location",
    LOCATIONS: "locations",
    MAP: "Map",
    MEDIAGUIDE: "Media guide",
    MEDIAGUIDES: "Media guides",
    MEDIAGUIDE_INTERACTIVE: "Interactive Media guide",
    MESSAGE_CAMERA_PERMISSION: "To continue, let the app access your camera",
    MESSAGE_LOCATION_PERMISSION: "To continue, let the app access your location",
    MINUTE: "min",
    MOST_POPULAR_ACTIVITIES: "Most popular activities",
    MORE_INFO: "More info",
    MORE_THAN: "More than",
    MORNING: "Morning",
    NEW_CONTENT: "New content",
    NO_BLE_CONNECTION: "Bluetooth disabled",
    NO_BLE_CONNECTION_MESSAGE: "Activate Bluetooth for full access to Guide Helsingborg",
    NO_INTERNET_CONNECTION: "No internet connection",
    NO_INTERNET_CONNECTION_MESSAGE: "Activate wifi for full access to Guide Helsingborg",
    NOT_FOUND_TITLE: "Oops!",
    NOT_FOUND_DESCRIPTION: "The page you are looking for does not exist",
    NOT_FOUND_CONTACT: "Please contact support:",
    OBJECT: "object",
    OBS: "Something went wrong",
    OFFLINE_CONTENT: "Offline content",
    OFFLINE_CONTENT_TITLE: "Downloaded",
    OK: "Ok",
    OPEN: "Open",
    OPEN_LINK: "Open link?",
    OPEN_IN_MAPS: "Open in Maps?",
    OPEN_TODAY: "Open today",
    ORGANIZERS: "Organizers",
    OUTDOOR: "Outdoor",
    PARKING: "Parking",
    PADEL_TRACKS: "Padeltracks",
    PLAYING: "Playing..",
    PLAYGROUNDS: "Playgrounds",
    READ_MORE: "Read more!",
    RESET: "Reset",
    RANDOMIZE: "Randomize",
    SCAN: "Scan",
    SCAN_QR_CODES: "Scan QR Codes",
    SCAN_QR_CODES_DESCRIPTION: "Here you can scan QR codes that are placed throughout Helsingborg. Aim your camera at the proper QR code to open the content in the app!",
    SCAN_QR_CODES_REMINDER: "When scanning, remember to have the same languague set in the app that corresponds to the guide's language. You can change language in settings.",
    SEARCH: "Search",
    SEARCH_AROUND: "Loading new content",
    SEARCH_MAIN: "Search for sights, places...",
    SEARCH_BY_NUMBER: "Search by id",
    SEE: "See",
    SEND: "Send",
    SETTINGS: "Settings",
    SHARE: "Share",
    SHARING_OVERLAY_TITLE: "Guide Helsingborg App",
    SHOP: "Shop",
    SHOPPING: "Shopping",
    SHOW_MAP: "Show map",
    SIGHTSEEINGS: "Sightseeings",
    SOMETHING_NEAR_BY: "There's something nearby",
    SEARCH_ALL_ACTIVITES: "Search all activities",
    SEARCH_ADDITIONAL_ACTIVITES: "Search additional activities",
    TAKE_ME_THERE: "Take me there",
    THEATER: "Theater",
    TIME_AND_DATE: "Time and date",
    TOPLIST: "Toplist",
    TOUR: "Tour",
    TUTORIAL: "Tutorial",
    UNKNOWN_TITLE: "Unknown title.",
    VIDEO: "Play video",
    VIEW_ALL: "View all",
    WITH: "with",
  },
  de: {
    ABOUT: "Über",
    ABOUT_GUIDE_APP: "Über Guide Helsingborg",
    ACCESS_TO_LOCATION: "Zugriff auf Location erlauben",
    ACCESSIBLE: "Handicap angepasst",
    ACTIVITY: "Aktivität",
    ACTIVITIES: "Aktivitäten",
    ADULT_ONLY: "Nur für Erwachsene",
    ANIMALS_ALLOWED: "Haustiere erlaubt",
    APP_NAME: "Guide Helsingborg",
    AFTERNOON: "Nachmittag",
    AR: "Wegweiser",
    AR_CAMERA_DISABLED: "Erlaube das App, die Kamera zu benutzen",
    AR_CHECKING: "Überprüft, ob die Funktion von Ihrem Telefon unterstützt wird",
    AR_HINT_ARRIVED: "Entecken Sie den Platz, indem Sie auf die Karte klicken oder den Audioguide anhören",
    AR_HINT_LOOK_DOWN: "Schau nach unten",
    AR_HINT_LOOK_LEFT: "Schau nach links",
    AR_HINT_LOOK_RIGHT: "Schau nach rechts",
    AR_HINT_LOOK_UP: "Schau nach oben",
    AR_HINT_TURN_AROUND: "Dreh dich um",
    AR_INTRO_BETA_BADGE: "",
    AR_INTRO_BUTTON: "Starten Sie die Führung",
    AR_INTRO_INSTRUCTIONS: "Halten Sie das Telefon auf und suchen Sie den gewünschten Ort in Ihrer Nähe.",
    AR_INTRO_TITLE: "Probieren Sie eine neue, unterhaltsame Art, Sofiero zu entdecken",
    AR_NOT_SUPPORTED_CALLOUT: "",
    AR_SUPPORTED: "Die Funktion wird von Ihrem Telefon unterstützt",
    AR_TRANSIENT: "Die Funktion wartet darauf, aktiviert zu werden",
    AR_UNKNOWN: "Diese Funktion wird von Ihrem Telefon nicht unterstützt",
    AR_UNSUPPORTED: "Diese Funktion wird von Ihrem Telefon nicht unterstützt",
    BATHING_AREAS: "Badeplätze",
    BETA_VERSION: "Beta version",
    BOOKING: "Buchung",
    CALENDAR: "Kalender",
    CALENDAR_FULL_DAY: "Den ganzen Tag",
    CALENDAR_NO_CONTENT: "Heute scheinen keine Veranstaltungen statt zu finden",
    CANCEL: "Stornieren",
    CINEMA: "Kino",
    CHANGES_REQUIRES_APP_RELOAD: "Förändringar kräver app reload",
    CHECK_CAMERA_SERVICE: "Kamera aktivieren",
    CHECK_LOCATION_SERVICE: "Location Service aktivieren",
    CHOOSE_LANGUAGE: "Sprache wählen",
    CHILDREN_FRIENDLY: "Kinderfreundlich",
    CITY_NAME: "Helsingborg",
    CLOSE: "Schließen",
    CLOSE_MORE_INFO: "Mehr Info schließen",
    CLOSE_TODAY: "Heute geschlossen",
    COMING_SOON: "Bald verfügbar",
    CONTACT_MAIL_ADRESS: "kontaktcenter@helsingborg.se",
    CONTACT_MAIL_SUBJECT: "Guide Helsingborg App",
    CONTACT_PHONE: "+4642105000",
    CONTACT_PHONE_DISPLAY: "042-10 50 00",
    CONTACT_US: "Kontakt",
    CONTENT_MISSING: "Kein Inhalt in Ihrer ausgewählten Sprache verfügbar",
    DATE: 'Zeit',
    DELETE_STORED_DATA: "DE Radera sparad media",
    DETAILED_INFORMATION: "Genaue Information",
    DISTANCE: "Distanz",
    DIRECTIONS: "Route",
    DOWNLOAD: "Download",
    DOWNLOADS: "Downloads",
    DOWNLOADED: "Offline verfügbar",
    DOWNLOADED_INFO: "Der Inhalt wird heruntergeladen und offline verfügbar. Gehen Sie zu Einstellungen, wenn Sie heruntergeladene Inhalte entfernen möchten.",
    DOWNLOADING: "Lädt herunter",
    DOWNLOADING_CANCEL: "Beenden",
    DOWNLOADING_PAUSE: "(berühren, um zu pausieren)",
    DOWNLOADING_RESUME: "Fortsetzen",
    EVENING: "Abend",
    EXHIBITIONS: "Ausstellungen",
    EXERCISE: "Übung",
    EXPERIENCES: "Erfahrungen",
    INDOOR: "Drinnen",
    INVALID_URL: "Ungültiger Link",
    INVALID_URL_MESSAGE: "Dieser Inhalt kann in der App nicht geöffnet werden",
    FILTER: "Filter",
    FILTER_TITLE: "Was willst Du heute machen",
    FILTER_SETTINGS: "Filter",
    FOR_CHILDREN: "Für Kinder",
    FROM_HERE: "von hier",
    FROM_YOU: "von dir",
    GO_BACK: "Geh zurück",
    GROCERIES: "Lebensmittelgeschäfte",
    GUIDE: "Guide",
    HI: "hi",
    HOME: "Heim",
    HOUR: "Stunde",
    INFO: "Info",
    INFORMATION: "Information",
    JUMP_OVER: "ÜBERSPRINGEN",
    LETS_GO: "LOS GEHT’S",
    LIST: "Liste",
    LISTEN: "Ton spielen",
    LOCATION: "Platz",
    LOCATIONS: "locations",
    MAP: "Karte",
    MEDIAGUIDE: "Media guide",
    MEDIAGUIDES: "Media guides",
    MEDIAGUIDE_INTERACTIVE: "Interaktiv Media guide",
    MESSAGE_CAMERA_PERMISSION: "Geben Sie das App zum Fortfahren Zugang auf Ihre Kamera",
    MESSAGE_LOCATION_PERMISSION: "Erlauben Sie der App Verwendung des Location Service  um fortzufahren.",
    MINUTE: "Minute (minut) Minuten (minuter)",
    MOST_POPULAR_ACTIVITIES: "Die beliebtesten aktivitäten",
    MORE_INFO: "Mehr Info",
    MORE_THAN: "Mehr als",
    MORNING: "Morgen",
    NEW_CONTENT: "Neuer Inhalt",
    NO_BLE_CONNECTION: "Kein Bluetooth",
    NO_BLE_CONNECTION_MESSAGE: "Um den Guide Helsingborg zu verwenden  ist Bluetooth notwendig",
    NO_INTERNET_CONNECTION: "Kein Netzwerk",
    NO_INTERNET_CONNECTION_MESSAGE: "Um den Guide Helsingborg zu verwenden ist eine Internetverbindung notwendig",
    NOT_FOUND_TITLE: "Hoppla!",
    NOT_FOUND_DESCRIPTION: "Die gesuchte Seite existiert nicht",
    NOT_FOUND_CONTACT: "Bitte wenden Sie sich an den Support:",
    OBJECT: "objekt",
    OBS: "Etwas ist schiefgegangen",
    OFFLINE_CONTENT: "Downloads",
    OFFLINE_CONTENT_TITLE: "Downloads",
    OK: "Ok",
    OPEN: "Öffnen",
    OPEN_LINK: "Verbindung öffnen?",
    OPEN_IN_MAPS: "Maps öffnen?",
    OPEN_TODAY: "Heute geöffnet",
    ORGANIZERS: "Veranstalter",
    OUTDOOR: "Draußen",
    PADEL_TRACKS: "Padel",
    PARKING: "Parken",
    PLAYING: "Spielen..",
    PLAYGROUNDS: "Spielplätze",
    RANDOMIZE: "Randomisieren",
    READ_MORE: "Weiterlesen!",
    RESET: "Zurücksetzen",
    SCAN: "Scan",
    SCAN_QR_CODES: "QR Codes Scannen",
    SCAN_QR_CODES_DESCRIPTION: "Hier können Sie QR-Codes scannen, die überall in Helsingborg platziert sind. Halten Sie Ihre Kamera über den QR-Code, um das richtige Erlebnis in der App zu öffnen!",
    SCAN_QR_CODES_REMINDER: "Denken Sie beim Scannen daran, in der App dieselbe Sprache eingestellt zu haben, die der Sprache des Reiseführers entspricht. Sie können die Sprache in den Einstellungen ändern.",
    SEARCH: "Suche",
    SEARCH_MAIN: "Suche nach Sehenswürdigkeiten, Orten ...",
    SEARCH_ALL_ACTIVITES: "Suche alle aktivitäten",
    SEARCH_ADDITIONAL_ACTIVITES: "Suche nach anderen aktivitäten",
    SEARCH_AROUND: "Suche läuft…",
    SEARCH_BY_NUMBER: "Nach ID suchen",
    SEE: "Siehe",
    SEND: "Abschicken",
    SETTINGS: "Einstellungen",
    SIGHTSEEINGS: "Sehenswürdigkeiten",
    SHARE: "Teilen",
    SHARING_OVERLAY_TITLE: "Guide Helsingborg App",
    SHOP: "Kaufen",
    SHOPPING: "Einkaufen",
    SHOW_MAP: "Karte zeigen",
    SOMETHING_NEAR_BY: "In der Nähe",
    SPORT: "Sport",
    TAKE_ME_THERE: "Weg finden",
    THEATER: "Theater",
    TIME_AND_DATE: "Zeit und datum",
    TOPLIST: "Bestenliste",
    TOUR: "Tour",
    TUTORIAL: "Tutorial",
    UNKNOWN_TITLE: "Unbekannter Titel",
    VIDEO: "Video spielen",
    VIEW_ALL: "Alle ansehen",
    WITH: "mit",
  },
};
