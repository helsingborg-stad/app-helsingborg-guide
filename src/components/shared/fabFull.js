...
const BOX_HEIGHT = Dimensions.get('window').height*(.36);
...
export default class Fab extends Component{
    ...
    displayFabChildrenBox(){
        if(!this.state.open)
            return (
                <View style={styles.childrenContainerClosed}>
                </View>
            );
    ...
    render(){
        ...
        let fabBoxStyle =this.state.open?{width:FULL_WIDTH,height:FULL_HEIGHT}:{backgroundColor:'rgba(0,0,0,0)'};
        return (
            <View style={[styles.fabBoxContainer,fabBoxStyle]}>
                <View style={{flex:1,position:'absolute',top:CENTER_HEIGHT+50,right:0}}>
                    <TouchableOpacity activeOpacity={0.88} onPress={this.toggleOpen.bind(this)}
                                      style={styles.parentFabContainer}>
