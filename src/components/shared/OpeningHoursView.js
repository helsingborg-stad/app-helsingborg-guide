...
function getOpeningHours(
  openingList: OpenHour[],
  expList: OpenHourException[],
  now: Date
) {
  ...
  if (expList && expList.length) {
    const exp = expList.find(
      item => now.toDateString() === new Date(item.date).toDateString()
    );
    if (exp) {
...
export default function OpeningHoursView(props: Props) {
  const opening = getOpeningHours(
    props.openHours,
    props.openHoursException,
    props.now
  );
  const text = opening || "";

  return <Text style={styles.hoursText}>{text}</Text>;
}
