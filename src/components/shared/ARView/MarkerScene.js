...
class MarkerScene extends Component<Props, State> {
  static getDerivedStateFromProps(props: Props, state: State) {
    ...
    const newMarkers = markers.map(marker => {
      ...
      const arrived = LocationUtils.hasArrivedAtDestination(
        userLocation,
        contentLocation
      );

      const distance = LocationUtils.getTravelDistance(
        userLocation.coords,
        contentLocation
      );
  ...
  onCameraTransformUpdate = (cameraTransform: any) => {
    ...
    if (active) {
      ...
      const angleDelta = MathUtils.limitPrecision(
        ((MathUtils.PI2 + cameraAngle - markerAngle) % MathUtils.PI2) *
          MathUtils.RAD_TO_DEG,
        2
      );

      // get the camera's rotation around the x-axis
      const verticalAngle = MathUtils.limitPrecision(
        Math.atan2(forwardY, forwardZ) * MathUtils.RAD_TO_DEG + 180,
        2
      );

      if (
        MathUtils.exceedsThreshold(
          currentHorizontalAngleDelta,
          angleDelta,
          ANGLE_THRESHOLD
        ) ||
      ...
      >
        {!isInitialized ? (
          ...
          <ViroNode>
            <ViroCamera active />
            {selectedMarker && (
              <AnimatedCharacter markerPosition={selectedMarker.position} />
            )}
            {markers.map(marker => (
              <Marker {...marker} />
            ))}
...
const mapState = (state: RootState, ownProps: ?Props) => {
  const {
    geolocation: { bearing, position }
  } = state;
  return { bearing, position, ...ownProps };
};

const mapDispatch = (dispatch: Dispatch) => ({
  dispatchCameraUpdateAngles: (cameraTransform: Object) =>
    dispatch(updateCameraAngles(cameraTransform))
});
