...
export default class OptionsView extends Component{
    ...
    animate(visible){
        ...
        if(visible)
            Animated.timing(this.state.animValue,{toValue:1,duration:200}).start();
        else
            Animated.timing(this.state.animValue,{toValue:0,duration:200}).start();
    ...
    render(){
        ...
        if(this.state.animValue){
            let translateAnim = this.state.animValue.interpolate({
                inputRange: [0, 1],
                outputRange: [MENU_WIDTH, 0]
            });

            let zIndexAnim = this.state.animValue.interpolate({
                inputRange: [0, 1],
