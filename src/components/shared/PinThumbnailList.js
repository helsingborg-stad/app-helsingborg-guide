...
export default class PinThumbnailList extends Component {
  displayObjects() {
    if (!this.props.contentObjects || Object.keys(this.props.contentObjects).length == 0) return;
    const contentObjects = this.props.contentObjects;
    ...
    const createThumbnail = function (key) {
      ...
      return (
        ...
        >
          <ContentThumbnailPin source={{ uri: imageUri }} imageWidth={width} imageHeight={height} checked={metric.isVisited || false}>
            {text}
          </ContentThumbnailPin>
    ...
    const leftList = leftKeys.map(key => createThumbnail(key));
    const rightList = rightKeys.map(key => createThumbnail(key));

    return (
      <View style={{ flex: 1, flexDirection: "row" }}>
        <View style={{ flex: 1, paddingLeft: 20, paddingRight: 10, paddingVertical: 20 }}>{leftList}</View>
        <View style={{ flex: 1, paddingLeft: 10, paddingRight: 20, paddingVertical: 20 }}>{rightList}</View>
      </View>
    );
  }
  ...
  render() {
    return <View style={styles.objectsContainer}>{this.displayObjects()}</View>;
  }
}
