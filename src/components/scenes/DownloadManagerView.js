...
import {View,Text,Image,ListView, StyleSheet, Button, Dimensions, TouchableOpacity,ProgressBarAndroid, LayoutAnimation, ScrollView, AsyncStorage} from 'react-native';
...
//########################################################
    ...
    toggleTask(id){
        if(this.downloadManager.isExist(id)){
            let task = this.downloadManager.getTaskById(id);
            if(task.isCanceled)
                this.downloadManager.resumeTask(task.id);
