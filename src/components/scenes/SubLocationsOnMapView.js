...
import { Platform,View, Text, ListView, StyleSheet, Navigator, TouchableHighlight,TouchableOpacity, Image, StatusBar,Button , Linking, AsyncStorage } from 'react-native';
...
//########################################3
    ...
    filterSubLocationsWithoutLocationObject(subLocations){
        ...
        return subLocations.filter(item=>{
            let location = item._embedded.location;
            return !(!item._embedded || !location.length || !location[0].id);
        });
    }

    buildState(_subLocations){
        let subLocations = this.filterSubLocationsWithoutLocationObject(_subLocations);
...
if(Platform.OS ==='ios')
    ...
    render() {
        if(!this.state.subLocations || this.state.subLocations.length<1)
            return null;
        ...
        return (
          <ViewContainer >
              ...
              <MapThumbnailsView items={this.state.subLocations}
                                 mapFlex={9}
