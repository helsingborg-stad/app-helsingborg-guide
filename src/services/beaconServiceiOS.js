...
let BeaconManager = NativeModules.IosBeacon;
...
updateBeaconDataInList(data){
    ...
    let beacon = this.convertRawBeaconDataIntoBeaconObject(data);
...
convertRawBeaconDataIntoBeaconObject(data){
    let beacon = JSON.parse(data);
        let cleanUid = beacon.uid.replace('<','').replace('>','').replace(/ /g,'');
    ...
    getTheClosest(beacons){
        if(!beacons || !beacons.length)
            return;
        let sorted = beacons.sort((a,b)=>parseFloat(a.distance) - parseFloat(b.distance));
        return sorted[0];
