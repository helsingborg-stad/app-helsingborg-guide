...
export class DownloadTasksManager{
    ...
    static getInstance(){
        if(!instance)
            instance = new DownloadTasksManager();
        return instance;
    }
//######################################
    ...
    loadExistingTasks(downloads){
        ...
        _.forEach(downloads,item=>{
            let task = this.createTask(item, true);
    ...
    createTask(data, resumed=false){
       ...
       // console.log('download:Creating a new task', data.id);
        ...
        let task = new DownloadTask(data);
        this.tasks.push(task);
        if(!resumed)
            store.dispatch(dActions.createTaskSuccess(task.getMeta()));
        return task;
    }
