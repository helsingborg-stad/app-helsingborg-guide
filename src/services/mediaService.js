...
export class MediaService{
    ...
    init(audio){
        ...
        this.fetchService.isExist(audio.url)
        .then(exist=>{
            let fullPath = this.fetchService.getFullPath(audio.url);
            if(exist)
    ...
    release(){
        ...
        (Platform.OS=='ios')?
            MediaPlayer.release1():MediaPlayer.release();
    ...
    getMeta(){
        if(!this.audio.hasAudio)
            return Promise.reject('Audio object is null');
    ...
    pauseUpdatingState(){if(!this.updatePaused)this.updatePaused=true}
    resumeUpdatingState(){if(this.updatePaused)this.updatePaused=false}
