...
const FetchService = {
  ...
  isUrl(str) {
    if (!FetchService.isString(str)) {
      return false;
    }
    const index = _.findIndex(Headers, header =>
      _.startsWith(_.toLower(str), header)
    );
    return !(index === -1);
  },

  isDownloadableUrl(str) {
    if (!FetchService.isUrl(str)) {
      return false;
    }
    const index = _.findIndex(MIME, ext =>
      _.endsWith(_.toLower(str), `.${ext}`)
    );
    return !(index === -1);
  },
  ...
  scanJsonTree(mObject) {
    ...
    keys.forEach(key => {
      ...
      if (FetchService.isObject(item)) {
        paths = paths.concat(FetchService.scanJsonTree(item));
      }
    });
  ...
  isExist(path, guideID) {
    ...
    if (guideID) {
      fullPath = FetchService.getFullPath(`${guideID}/${path}`);
    }

    fullPath = stripTrailingSlash(fullPath);
    const exists = RNFetchBlob.fs.exists(
      Platform.OS === "android" ? `file://${fullPath}` : fullPath
    );
    return exists;
  },
